    ...
    S: Subscriber + for<'span> LookupSpan<'span> + Send + Sync,
{
    ...
    fn get_callsite(&self, data: EventOrSpan<'_, '_, S>) -> Callsite {
    ...
    fn get_root_id(span: SpanRef<'_, S>) -> u64 {
        span.scope()
            .from_root()
            .take(1)
            .next()
    ...
    fn enter_span(&self, span: SpanRef<'_, S>, ts: f64) {
    ...
    fn exit_span(&self, span: SpanRef<'_, S>, ts: f64) {
        let callsite = self.get_callsite(EventOrSpan::Span(&span));
        let root_id = match self.trace_style {
            TraceStyle::Async => Some(ChromeLayer::get_root_id(span)),
            _ => None,
